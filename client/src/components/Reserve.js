...
const Reserve = () => {
  ...
  const [allavailabletimes, setAvailableTimes] = useState([
    "8:00 AM",
    "10:00 AM",
    "12:00 PM",
    "2:00 PM",
      ...
      if (response.data.message) {
        ...
        if (numberofGuest <= 8) {
          ...
          for (var key in allavailableTables) {
            ...
            if (numGuest <= 0) {
              var table =
                "We will sit you in multiple tables of size: " + tableSize;
              setTableStatus(table);
              notEnough = false;
              break;
            }
    ...
    <div>
      {!loggedIn && (
        <div className="container">
          ...
          <p>
            {" "}
            Please consider registering before reserving your table. There is so
            many many benefits that it doesnt make sense not to register!!!
          </p>
        </div>
      )}
